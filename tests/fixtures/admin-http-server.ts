...
createServer(async (req, res) => {
  ...
  if (url.pathname === "/_test/state") {
    res.setHeader("Content-Type", "application/json"); res.end(JSON.stringify({ audit: backend.audit, emails: backend.emails, writes: backend.writes, uploads: backend.uploads, records: backend.records })); return;
  }
