...
import { ADMIN_OTP_SECONDS, isAllowedAdmin, normalizeAdminEmail } from "@/modules/admin/policy";
...
export async function sendAdminOtp(email: string): Promise<{ challengeId: string; cooldownSeconds: number }> {
  ...
  await writeAdminValue(name, {
    email: adminEmail, digest: adminRef("otp:" + challengeId, otp),
  ...
  try {
    ...
    await sendOtpEmail(emailLeadId, adminEmail, Number(otp), fetch, { ttlSeconds: ADMIN_OTP_SECONDS, admin: true });
  } catch {
